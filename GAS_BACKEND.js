 ...
 *  unlockedTitles | claimedChallenges | createdAt | updatedAt
 *
 *  帳號規則：
...
// ── 初始化 Sheet（第一次執行，或 sheet 不存在時自動建立） ───────────────────
function setupSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) {
  ...
  const headers = ["id","username","email","passwordHash","avatarPreset","avatarUrl","bio","activeTitle","level","coins","habits","friendIds","unlockedTitles","claimedChallenges","pendingFriendRequests","createdAt","updatedAt"];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  sheet.setFrozenRows(1);
  sheet.getRange(1, 1, 1, headers.length)
    .setBackground("#C1644B")
    .setFontColor("#ffffff")
    .setFontWeight("bold");
